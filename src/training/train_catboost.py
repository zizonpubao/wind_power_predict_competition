"""CatBoost ensemble-candidate training pipeline: per-KPX-group Optuna-tuned
CV scoring (via the official ``competition_score``) + a final full-data
refit, written to ``experiments/<run_id>/`` -- the CatBoost counterpart to
``src/training/tune_hyperparams.py`` (LightGBM) / ``src/training/train_xgb.py``
(XGBoost).
...
from src.models.catboost_model import DEFAULT_EARLY_STOPPING_ROUNDS, DEFAULT_PARAMS, GroupCatBoostModel
...
# Current-best LightGBM run to diff the CatBoost scores against in the printed
...
def _suggest_params(trial: optuna.Trial) -> dict[str, Any]:
    ...
    CatBoost regression, with a 20-trial budget (lighter than LightGBM's 40 --
    see module docstring). ``bootstrap_type`` stays fixed at
    ``GroupCatBoostModel``'s default (``"Bernoulli"``, required for
    ``subsample`` to be a valid parameter at all) rather than being searched.
    """
    return {
...
def main() -> str:
    ...
    parser = argparse.ArgumentParser(description="Optuna-tuned CatBoost training for the per-group ensemble.")
    ...
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S") + "_catboost_tuned"
    ...
    for kpx_group in KPX_GROUPS:
        logger.info("=== Tuning %s (%d trials, CatBoost) ===", kpx_group, args.n_trials)
    ...
    print(f"\n=== LightGBM ({BASELINE_RUN_ID}) vs CatBoost ({run_id}) CV Score (mean across folds) ===")
    header = f"{'group':<16}{'lgbm':>12}{'catboost':>12}{'delta':>12}"
    print(header)
    print("-" * len(header))
    cb_scores = {g: all_results[g]["agg_metrics"]["score_mean"] for g in KPX_GROUPS}
    for g in KPX_GROUPS:
