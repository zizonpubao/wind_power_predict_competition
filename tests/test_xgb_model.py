"""Unit tests for src/models/xgb_model.py (GroupXGBModel wrapper).

Uses a tiny synthetic tabular dataset -- not the real feature parquets (that
end-to-end run is covered by actually executing src/training/train_xgb.py,
not by this suite). Mirrors tests/test_lgbm_model.py's structure/coverage for
the subset of behavior GroupXGBModel shares with GroupLGBMModel (capacity
clip, best_iteration_, fit/predict signature, pickling).
"""
import joblib
import numpy as np
import pandas as pd
...
def test_fit_predict_runs_and_returns_expected_shape():
    ...
    model = GroupXGBModel(capacity_kwh=21_600.0, n_estimators=50)
...
def test_fit_with_eval_set_uses_early_stopping():
    ...
    model = GroupXGBModel(capacity_kwh=21_600.0, n_estimators=500)
    model.fit(X_train, y_train, eval_set=(X_val, y_val), early_stopping_rounds=5)

    # best_iteration_ should be recorded (an int, actual tree count -- see
    # module docstring's 0-indexed-vs-count convention) when eval_set is given.
    assert model.best_iteration_ is not None
    assert model.best_iteration_ <= 500


def test_fit_without_eval_set_leaves_best_iteration_none():
    X, y = _make_synthetic_data()
    model = GroupXGBModel(capacity_kwh=21_600.0, n_estimators=20)
    model.fit(X, y)
...
def test_predict_clips_to_capacity_range():
    ...
    tiny_capacity = 1.0
    model = GroupXGBModel(capacity_kwh=tiny_capacity, n_estimators=20)
    model.fit(X, y)
...
def test_predict_never_returns_negative_even_with_negative_targets():
    ...
    model = GroupXGBModel(capacity_kwh=21_600.0, n_estimators=30)
    model.fit(X, y)
    preds = model.predict(X)
    assert (preds >= 0.0).all()
...
def test_capacity_kwh_stored_and_default_params_merged_with_overrides():
    model = GroupXGBModel(capacity_kwh=21_000.0, max_depth=3, n_estimators=10)
...
def test_model_pickles_round_trip(tmp_path):
    ...
    dump_path = tmp_path / "xgb_model.joblib"
...
def test_repeated_fit_calls_do_not_warm_start():
    """Calling .fit() twice on the same wrapper instance (e.g. one CV fold's
    fit followed by a later full-data refit reusing the wrapper) must not
    accumulate trees across the two calls -- each .fit() should build a fresh
    XGBRegressor. Compares final tree count against a model fit fresh once.
    """
    X, y = _make_synthetic_data()
    ...
    fresh = GroupXGBModel(capacity_kwh=21_600.0, n_estimators=30)
    fresh.fit(X, y)

    assert model.model_.get_booster().num_boosted_rounds() == fresh.model_.get_booster().num_boosted_rounds()
