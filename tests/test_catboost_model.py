"""Unit tests for src/models/catboost_model.py (GroupCatBoostModel wrapper).

Uses a tiny synthetic tabular dataset -- not the real feature parquets (that
end-to-end run is covered by actually executing src/training/train_catboost.py,
not by this suite). Mirrors tests/test_lgbm_model.py's structure/coverage for
the subset of behavior GroupCatBoostModel shares with GroupLGBMModel (capacity
...
def test_fit_predict_runs_and_returns_expected_shape():
    ...
    model = GroupCatBoostModel(capacity_kwh=21_600.0, iterations=50)
...
def test_fit_with_eval_set_uses_early_stopping():
    ...
    model = GroupCatBoostModel(capacity_kwh=21_600.0, iterations=500)
...
def test_fit_without_eval_set_leaves_best_iteration_none():
    X, y = _make_synthetic_data()
    model = GroupCatBoostModel(capacity_kwh=21_600.0, iterations=20)
    model.fit(X, y)
...
def test_predict_clips_to_capacity_range():
    ...
    tiny_capacity = 1.0
    model = GroupCatBoostModel(capacity_kwh=tiny_capacity, iterations=20)
    model.fit(X, y)
...
def test_predict_never_returns_negative_even_with_negative_targets():
    ...
    model = GroupCatBoostModel(capacity_kwh=21_600.0, iterations=30)
    model.fit(X, y)
    preds = model.predict(X)
    assert (preds >= 0.0).all()
...
def test_capacity_kwh_stored_and_default_params_merged_with_overrides():
    model = GroupCatBoostModel(capacity_kwh=21_000.0, depth=3, iterations=10)
...
def test_model_pickles_round_trip(tmp_path):
    ...
    dump_path = tmp_path / "catboost_model.joblib"
